...
void onAppSpecialized(const ZnHyosAppSpecializeArgs *args) {
    if (args == nullptr) {
        statusLine("S3 callback args=null pid=%d uid=%d", getpid(), getuid());
        return;
    }

    statusLine("S3 callback package=%s process=%s seinfo=%s pid=%d uid=%d",
    ...
    if (args->package_name != nullptr && strcmp(args->package_name, kTargetPackage) == 0) {
        statusLine("S3 WEATHER_MATCH pid=%d uid=%d", getpid(), getuid());
    }
}
...
void onModuleLoaded(void *, const ZygiskNextAPI *api) {
    // Open once while still inside the privileged hyos_spawner host. The fd is
    // intentionally inherited by specialized children, so callback diagnostics
    // do not depend on app sandbox paths or post-specialization SELinux access.
    gStatusFd = open(kStatusPath, O_WRONLY | O_CREAT | O_TRUNC, 0600);
    statusLine("S3 onModuleLoaded pid=%d uid=%d fd=%d errno=%d", getpid(), getuid(), gStatusFd,
               gStatusFd < 0 ? errno : 0);

    if (api == nullptr) {
        statusLine("S3 api=null");
        return;
    }
    if (api->getRuntime == nullptr) {
        statusLine("S3 getRuntime=null");
        return;
    }

    const ZygiskNextRuntime *runtime = api->getRuntime();
    if (runtime == nullptr) {
        statusLine("S3 runtime=null");
        return;
    }

    statusLine("S3 runtime type=%d api=%d register=%p", static_cast<int>(runtime->type),
               runtime->api_version, reinterpret_cast<void *>(runtime->registerModule));

    if (runtime->type != ZN_RUNTIME_HYOS) {
        statusLine("S3 runtime_type_mismatch expected=%d actual=%d", static_cast<int>(ZN_RUNTIME_HYOS),
                   static_cast<int>(runtime->type));
        return;
    }
    if (runtime->api_version < ZYGISK_NEXT_HYOS_API_VERSION) {
        statusLine("S3 runtime_api_too_old required=%d actual=%d", ZYGISK_NEXT_HYOS_API_VERSION,
                   runtime->api_version);
        return;
    }
    if (runtime->registerModule == nullptr) {
        statusLine("S3 registerModule=null");
        return;
    }

    int rc = runtime->registerModule(&kHyosModule);
    statusLine("S3 registerModule rc=%d target_api=%d", rc, ZYGISK_NEXT_HYOS_API_VERSION);
}
